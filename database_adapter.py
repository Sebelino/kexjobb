...
def insert(c,args):
    row = [args[0],int(args[1]),int(args[2]),None]
    c.execute("INSERT INTO classifications VALUES (?,?,?,?)",row)
    return 'Insertion successful.'

# Deletes the rows satisfying the condition.
def delete(c,condition):
    c.execute("DELETE FROM classifications WHERE %s"% condition[0])
...
def select(c,condition):
    if len(condition) == 0:
        ...
        c.execute("SELECT * FROM classifications WHERE %s"% condition[0])
